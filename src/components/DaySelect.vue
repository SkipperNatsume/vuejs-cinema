<template>
    <div id="day-select">
        <ul class="days">
            <li v-bind:class="{day:true, active: isActive(day)}" v-for="day in days" class="day" v-on:click="selectDay(day)">{{formatDay(day)}}</li>
...
<script>
    export default {
        ...
        methods: {
            ...
            changeDay(change) {
                let newDay = this.$moment(this.selected).add(change, 'days')//this is actually a method from $moment
